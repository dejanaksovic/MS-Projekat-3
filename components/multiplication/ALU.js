export default function ALU() {
    ...
    this.stepFunc = (product, multiplicand, multiplier) => {
        ...
        if (this.iteration < multiplier.length) {
            if (this.currStep != 1) {
                if (this.currStep == 0) {
                    returnVal[1] = (this.iteration + 1) + ":" + (this.currStep + 1) + " Multiplier pomeren u desno."
                    this.undoStack.push(['multiplier', multiplier])
                } else {
                    returnVal[1] = (this.iteration + 1) + ":" + (this.currStep + 1) + " Multiplicand pomeren u levo."
                    this.iteration++
            ...
            if (this.currStep == 1 && this.carryOut == '1') {
                ...
                returnVal[1] = (this.iteration + 1) + ":" + (this.currStep + 1) + " Multiplicand je dodat na Product."
            } else if (this.currStep == 1 && this.carryOut == '0') {
                returnVal[1] = (this.iteration + 1) + ":" + (this.currStep + 1) + " Sabiranje Multiplicand i Product komponenti nije omogućeno jer je izneti bit Multiplier komponente '0'."
            }
