    ...
    this.stepFunc = (productandmultiplier, multiplicand) => {
        ...
        if(this.iteration < multiplicand.length){
            ...
            if(carryOut == '1' && this.currStep == 0){
                ...
                returnValue[1] = "Multiplicand dodat na prvu polovinu Product/Multiplier komponente", returnValue[0]
                this.undoStack.push(['productandmultiplier', productandmultiplier])
            }else if(this.currStep == 1){
                this.steps[this.currStep]()
                returnValue[1] = "Product/Multiplier komponenta se pomera desno"
                this.iteration++
                this.undoStack.push(['productandmultiplier', productandmultiplier])
            }else{
                returnValue[1] = "Jer je LSB Product/Multiplier komponente '0' sabiranje se neće izvršiti."
                this.undoStack.push(['productandmultiplier', productandmultiplier])
            }
